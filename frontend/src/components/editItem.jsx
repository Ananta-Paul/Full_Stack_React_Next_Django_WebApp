...
const EditItem = ({ stock }) => {
  //console.log(stock);
  const [editable, setEditable] = useState(false);
  const [status, setStatus] = useState({
    status: stock?.ustatus,
    score: stock?.uscore,
    episodes: stock?.uepisodes,
  });
  const handleClick = async (e) => {
    ...
    setEditable(false);
    if (
      status.status != stock?.ustatus ||
      status.episodes !== stock?.uepisodes ||
      status.score !== stock?.uscore
    ) {
      const res = await editItem({ updateDetails: status, id: stock._id });
      if (res !== true)
        setStatus({
          status: stock?.ustatus,
          score: stock?.uscore,
      ...
      {editable ? (
        ...
        <>
          ...
          <p>
            ...
            {status.episodes + "/" + stock?.episodes}
          </p>
        </>
      )}
      <button
